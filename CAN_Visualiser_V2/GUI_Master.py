...
class ComGui():
    def __init__(self, root, serial, data):
        ...
        self.btn_refresh = customtkinter.CTkButton(self.frame, text="Refresh",
                                  width=10,  command=self.com_refresh)
        self.btn_connect = customtkinter.CTkButton(self.frame, text="Connect",
    ...
    def ComOptionMenu(self):
        ...
        self.drop_com = customtkinter.CTkOptionMenu(
            self.frame, variable = self.clicked_com, values = self.serial.com_list, width =80,command=self.connect_ctrl,dynamic_resizing=False)
    ...
    def baudOptionMenu(self):
        ...
        self.drop_baud = customtkinter.CTkOptionMenu(
            self.frame, variable=self.clicked_bd, values=bds, width =80, command=self.connect_ctrl,dynamic_resizing=False)
...
class ConnGUI():
    def __init__(self, root, serial, data):
        ...
        self.btn_start_stream = customtkinter.CTkButton(self.frame, text="Start", state="disabled",
                                       width=5, command=self.start_stream)

        self.btn_stop_stream = customtkinter.CTkButton(self.frame, text="Stop", state="disabled",
                                      width=5, command=self.stop_stream)

        self.btn_add_chart = customtkinter.CTkButton(self.frame, text="+", state="disabled",
                                    width=5, fg_color="#098577",
                                    command=self.new_chart)

        self.btn_kill_chart = customtkinter.CTkButton(self.frame, text="-", state="disabled",
                                     width=5, fg_color="#CC252C",
                                     command=self.kill_chart)
        self.save = False
        self.SaveVar = IntVar()
        self.save_check = customtkinter.CTkCheckBox(self.frame, text="Save data", variable=self.SaveVar,
...
class DisGUI():
    ...
    def AddMasterFrame(self):
        self.frames.append(LabelFrame(
            self.root, text=f"Display Manager - {len(self.frames)+1}", padx=5, pady=5))

        # self.frames.append( MyFrame(master=self.root,text = f"Display Manager - {len(self.frames)+1}") )
        # self.frame.grid( padx=5, pady=5)
    ...
    def AddBtnFrame(self):
        ...
        self.ControlFrames[self.totalframes].append(
            customtkinter.CTkButton(self.ControlFrames[self.totalframes][0], text="+", width=btnW, height=btnH, command=partial(self.AddChannel, self.ChannelFrame[self.totalframes])))
        self.ControlFrames[self.totalframes][1].grid(
            column=0, row=0, padx=5, pady=5)
        self.ControlFrames[self.totalframes].append(
            customtkinter.CTkButton(self.ControlFrames[self.totalframes][0], text="-", width=btnW, height=btnH, command=partial(self.DeleteChannel, self.ChannelFrame[self.totalframes])))
    ...
    def AddChannel(self, ChannelFrame):
        ...
        if len(ChannelFrame[0].winfo_children()) < 8:
            ...
            Ch_btn = customtkinter.CTkCheckBox(NewFrameChannel, text = " ",variable=self.ViewVar[ChannelFrame[1]][len(self.ViewVar[ChannelFrame[1]])-1],
                                 onvalue=1, offvalue=0, width = 1, height=1)
    ...
    def ChannelOption(self, Frame, ChannelFrameNumber):
        ...
        drop_ch = customtkinter.CTkOptionMenu(Frame, variable=self.OptionVar[ChannelFrameNumber][len(
            self.OptionVar[ChannelFrameNumber])-1], values=bds,width = 70, dynamic_resizing=False)
    ...
    def ChannelFunc(self, Frame, ChannelFrameNumber):
        ...
        self.FunVar[ChannelFrameNumber][len(
            self.OptionVar[ChannelFrameNumber])-1].set(bds[0])
        drop_ch = customtkinter.CTkOptionMenu(Frame, variable=self.FunVar[ChannelFrameNumber][len(
            self.OptionVar[ChannelFrameNumber])-1], values=bds,width = 70, dynamic_resizing=False)
        drop_ch.configure(width=50)
